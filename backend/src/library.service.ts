...
export class LibraryService {
  ...
  private readAssetsFromDir(libraryDir: string, assetType: string): any[] {
    try {
      ...
      console.log(
        `[LIBRARY] Reading ${Object.keys(idToPath).length} tracked files for type ${assetType}`,
      );
      ...
      for (const [assetId, relativePath] of Object.entries(idToPath)) {
        try {
          ...
          console.error(
            `[LIBRARY] Error reading file for asset ${assetId}:`,
            err,
          );
        }
      }
      ...
      console.log(
        `[LIBRARY] Found ${entities.length} entities of type ${assetType}`,
      );
      return entities;
    } catch (error) {
      console.error(
        `[LIBRARY] Error reading assets of type ${assetType}:`,
        error,
      );
      return [];
    }
          ...
          if (stat.isFile()) {
            try {
              ...
              if (fs.existsSync(libraryDir)) {
                console.log(
                  `Library ${library.id} already migrated, skipping.`,
                );
                continue;
              }
    ...
    for (const entry of entries) {
      if (entry.isDirectory()) {
        try {
          ...
          console.error(
            `Error reading library directory ${entry.name}:`,
            error,
          );
        }
      }
  ...
  getLibrary(libraryId: string): Library {
    ...
    if (!fs.existsSync(metadataFilePath)) {
      console.error(
        `[LIBRARY] Metadata file not found for library "${libraryId}"`,
      );
      throw new NotFoundException(`Library with ID "${libraryId}" not found`);
    }
    ...
    console.log(
      `[LIBRARY] Loading library "${metadata.name}" (${metadata.id})`,
    );
    ...
    const oldSpells = this.readEntityCollection(
      path.join(libraryDir, 'spells'),
    );
    const oldSkills = this.readEntityCollection(
      path.join(libraryDir, 'skills'),
    );
    const oldStatusEffects = this.readEntityCollection(
      path.join(libraryDir, 'status-effects'),
    );
    const oldMacroActions = this.readEntityCollection(
      path.join(libraryDir, 'macro-actions'),
    );
    const oldShops = this.readEntityCollection(path.join(libraryDir, 'shops'));

    console.log(
      `[LIBRARY] Old format - Items: ${oldItems.length}, Runes: ${oldRunes.length}, Spells: ${oldSpells.length}, Skills: ${oldSkills.length}`,
    );
    ...
    const newStatusEffects = this.readAssetsFromDir(
      libraryDir,
      'status-effect',
    );
    ...
    console.log(
      `[LIBRARY] New format - Items: ${newItems.length}, Runes: ${newRunes.length}, Spells: ${newSpells.length}, Skills: ${newSkills.length}, Shops: ${newShops.length}`,
    );
    ...
    console.log(
      `[LIBRARY] Final counts - Items: ${library.items.length}, Runes: ${library.runes.length}, Spells: ${library.spells.length}, Skills: ${library.skills.length}, Shops: ${library.shops.length}`,
    );
    ...
    const updatedMetadata = {
      ...
      isPublic:
        updates.isPublic !== undefined
          ? updates.isPublic
          : existingMetadata.isPublic,
      author:
        updates.author !== undefined ? updates.author : existingMetadata.author,
    };
