...
export class StressTestService {
  ...
  async generateStressTestData(config: {
    ...
    const availableImages =
      config.imageIds && config.imageIds.length > 0
        ? config.imageIds
  ...
  private async storeSampleImages(): Promise<string[]> {
    ...
    if (imageIds.length === 0) {
      console.warn(
        '[STRESS TEST] No sample images could be stored. Characters will be created without portraits.',
      );
    }
  ...
  private generateCharacter(id: string, availableImages: string[]): any {
    const name =
      this.randomFrom(this.names) +
      ' ' +
      this.randomFrom(['the Brave', 'the Wise', 'the Swift', 'the Strong', '']);
    ...
    return {
      ...
      portrait:
        availableImages.length > 0 ? this.randomFrom(availableImages) : '',
      worldName: '',
  ...
  private generateSpells(count: number, availableImages: string[]): any[] {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const description = this.randomFrom(this.spellDescriptions)
        ...
        .replace(
          '{area}',
          this.randomFrom(['5-foot', '10-foot', '20-foot', '30-foot']),
        )
        .replace('{value}', this.randomInt(10, 50).toString())
        .replace(
          '{duration}',
          this.randomFrom(['1 minute', '10 minutes', '1 hour', '8 hours']),
        )
        .replace(
          '{creature}',
          this.randomFrom([
        ...
        .replace(
          '{sacrifice}',
          this.randomFrom(['health', 'sanity', 'life force']),
        );
      ...
      spells.push({
        ...
        spellCastingTime: this.randomFrom([
          '1 action',
          '1 bonus action',
          '1 minute',
          '10 minutes',
        ]),
        spellRange: this.randomFrom([
          'Self',
          'Touch',
          '30 feet',
          '60 feet',
  ...
  private generateRunes(count: number, availableImages: string[]): any[] {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const description = this.randomFrom(this.runeDescriptions)
        ...
        .replace(
          '{element}',
          this.randomFrom(['fire', 'ice', 'lightning', 'earth', 'wind']),
        )
        .replace(
          '{effect}',
          this.randomFrom([
        ...
        .replace(
          '{title}',
          this.randomFrom(['Ancient', 'Wise', 'Terrible', 'Merciful', 'Cruel']),
        )
        .replace(
          '{deity}',
          this.randomFrom(['Thor', 'Odin', 'Loki', 'Freya', 'Baldur']),
        )
  ...
  private generateSkills(count: number): any[] {
    ...
    for (let i = 0; i < count; i++) {
      ...
      skills.push({
        ...
        skillProficiency: this.randomFrom([
          'None',
          'Proficient',
          'Expert',
          'Master',
