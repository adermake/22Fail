...
function runeSrc(rune: RuneBlock): string {
  const drawing = rune.drawing ?? '';
  if (!drawing) return '';
  return drawing.startsWith('data:image') ? drawing : `/api/images/${drawing}`;
}

function runeGlow(rune: RuneBlock): string {
  return HEX.test(rune.glowColor ?? '') ? rune.glowColor! : '#8b5cf6';
}

/** Case/whitespace-insensitive lookup by name. */
export function findRune(env: RulebookEnv, name: string): RuneBlock | undefined {
  const wanted = name.trim().toLowerCase();
  return (env.context?.runes ?? []).find((r) => (r.name ?? '').trim().toLowerCase() === wanted);
}

function runeTitle(rune: RuneBlock): string {
...
export function runeTile(rune: RuneBlock): string {
  const src = runeSrc(rune);
  const art = src
    ? `<img class="rb-rune-img" src="${esc(src)}" alt="" loading="lazy">`
    : `<span class="rb-rune-img rb-rune-img--empty" aria-hidden="true"></span>`;
  return (
    `<figure class="rb-rune" style="--rb-rune-glow:${runeGlow(rune)}"${runeTitle(rune)}>` +
    `${art}<figcaption class="rb-rune-name">${esc(rune.name ?? '')}</figcaption></figure>`
  );
}
...
export function runeChip(env: RulebookEnv, name: string): string {
  ...
  const src = runeSrc(rune);
  const art = src ? `<img class="rb-runechip-img" src="${esc(src)}" alt="" loading="lazy">` : '';
  return (
    `<span class="rb-runechip" style="--rb-rune-glow:${runeGlow(rune)}"${runeTitle(rune)}>` +
    `${art}<span class="rb-runechip-name">${esc(rune.name ?? '')}</span></span>`
  );
}
