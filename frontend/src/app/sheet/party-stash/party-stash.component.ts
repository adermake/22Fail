...
import { stackAmount, withAmount } from '../../utils/item-stack.util';
...
export class PartyStashComponent implements OnInit {
  ...
  /** Hand part of a pile (by default all of it) to the party. */
  async give(item: ItemBlock, count?: number): Promise<void> {
    const index = (this.sheet.inventory ?? []).indexOf(item);
    if (index < 0) return;

    const have = stackAmount(item);
    const amount = Math.max(1, Math.min(count ?? have, have));
    const unit = withAmount(item, amount);
    ...
    if (amount < have) {
      inventory[index] = withAmount(item, have - amount);
    } else {
