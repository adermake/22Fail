 ...
 *
 * **Anything unrecognised is a symbol, not clutter.** This started the other way round, on the
 * reasoning that a stray crate hiding in a drawer beats a washing line among the mountains.
 * That was the wrong way to be wrong. A hand-written vocabulary cannot cover 1200 names from
 * a dozen packs, so the fall-through is not a rare case — it caught `Arcane_Floating_Sanctuary`,
 * `Arcane_Magic_Portal`, `Mysterious_Pyramid` and every other building nobody thought to list,
 * and filed them where they would never be looked for. The assets were bought to be used;
 * hiding one is the more expensive mistake, and a misplaced crate is visible and harmless.
 *
 * So only a *positive* match for clutter reaches the props drawer. `misc` is the default and
