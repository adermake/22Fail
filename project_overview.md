...
- **Bauplan** (`shared/construct-editor/`): Vollbild-Montageansicht. Links das Inventar-Regal (lose Konstrukte), rechts der Baum. Jede Verbindung trägt ihre Kosten (`+1`, `+2`, …) als Badge, der Kopf zeigt Komplexität gegen freien Fokus. Abnehmen per ✕ am Knoten, immer erlaubt.
- **Layout = Auto-Layout + Verschiebung.** Gespeichert wird auf dem Item nur ein *Versatz* (`bauplanDX/DY`), nie eine absolute Position. Dadurch bleibt der aufgeräumte Baum die Grundlage: unverschobene Maschinen ordnen sich weiter selbst, ein Teil behält seinen Versatz wenn Geschwister dazukommen, und **ein Knoten nimmt seinen Unterbaum mit**, weil Kinder den Versatz ihrer Vorfahren erben. Die Werte reisen im selben JSON-Teilbaum mit (Containment) — kein eigener Sync. „Anordnen" im Kopf löscht alle Versätze. Rein kosmetisch, keine Regel liest sie.
- **Werkbank**: Teile lassen sich auch auf *freie Fläche* legen — dann liegen sie unverbunden auf dem Canvas (absolute `bauplanX/Y`, nicht der Versatz) und werden per Kabel angeschlossen wie im Runen-Editor. Abnehmen legt ein Teil genau dort ab, wo es hing, statt es ins Regal zu beamen; „↩" am Knoten schickt es zurück ins Regal. Beim Anschließen werden `bauplanX/Y` gelöscht, weil die Position dann aus dem Baum kommt. Beim Öffnen entscheidet allein die Anwesenheit von `bauplanX/Y`, ob ein loses Teil auf der Werkbank oder im Regal landet.
- **Drag vs. Klick**: `DRAG_THRESHOLD` (4px) trennt beides; nach einem echten Drag unterdrückt `suppressClick` den nachfolgenden Klick, sonst würde das Loslassen das Teil in der Hand anstecken. `mousedown` macht `preventDefault()`, sonst startet der Browser ein Text-Drag aus der Knoten-Beschriftung und verschluckt den Mousemove-Strom.
- **Bewegungen NIE durch Kopieren der Wurzel veröffentlichen.** Positionen werden am Item mutiert; publiziert wird über den Zähler `revision`. `working.set({...root})` sah funktionierend aus, weil Kinder über das kopierte `sockets`-Array geteilt bleiben — die Wurzel selbst wurde aber ersetzt, sodass ein laufender Drag auf ein verwaistes Objekt schrieb: **der Wurzelknoten ließ sich genau ein Frame weit ziehen**. Regressionstest: „keeps moving the ROOT across successive mousemoves".
- **Verbinden geht von beiden Seiten** (wie im Runen-Editor): Teil anklicken/ziehen → auf Knoten oder Raute ablegen, ODER Raute anklicken → Kabel hängt am Cursor → Teil im Regal anklicken. Ein Klick auf einen **Knoten** nimmt den ersten freien Anschluss; die Raute selbst hat eine 34px-Trefferfläche bei 13px Optik.
- **`ngOnChanges` darf NUR auf die Identität von `root` reagieren, nie auf `pool`.** Der Parent liefert `pool` aus einem Getter, also bei jedem Change-Detection-Lauf ein neues Array — ein Re-Seed darauf löschte das aufgenommene Teil zwischen zwei Klicks, und Anstecken tat scheinbar gar nichts. Regressionstest: „survives change detection" in `construct-editor.component.spec.ts`.
